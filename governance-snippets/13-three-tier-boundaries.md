...
**何时用**：项目有"无条件必做"和"先问后做"和"绝对禁止"三种强度的规则；想取代 [04-dont-do-pairing](04-dont-do-pairing.md) 的两层简化版。放在 AGENTS.md 顶部（紧邻"任务分流"上方），承担首屏非协商项的载体。
**占位符**：命令 / 路径 / 契约文件按项目替换。
...
- 项目无 escalation / ask_user 流程：保留"始终"+"禁止"两层，删"先问后做"（退化为 04 风格）。
- 团队接受 04 简化版：用 [04-dont-do-pairing](04-dont-do-pairing.md)；本片段是 04 的进阶。
