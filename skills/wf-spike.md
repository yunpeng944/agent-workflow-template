...
**Stage prompt 来源**：从对应 `===== BEGIN STAGE-N-<ROLE> PROMPT =====` ↔ `===== END … =====` 之间复制。spike code 落 `/tmp/spike-<topic>/`（**不**在主仓 / **不**在 worktree，避免被误 git add）。
...
**降级**：用户已想清楚单一子问题 → `single-probe`（仍走 Stage 1 但只产 1 个 probe，可极简通过）；不需跑代码就能答（纯读源码 / 文档） → `learning-only`（Stage 2 跑但禁止写 spike code，OBSERVATIONS 引证文档 / 源码）。
...
````
===== BEGIN STAGE-1-PROBE-DESIGNER PROMPT =====
...
## Probes（≤ 3 个；single-probe 模式下恰好 1 个）
...
```
===== END STAGE-1-PROBE-DESIGNER PROMPT =====
````
...
> **强制环境**：本 stage **必须**在 `git worktree add /tmp/wt-spike-<run-id>` 隔离的 worktree 内跑；spike code 写到 `/tmp/spike-<topic>/`（**不在** worktree 内 / **不在** 主仓内，避免被误 git add）。Stage 2 输出 Environment 段必填 worktree 路径（`git rev-parse --show-toplevel` 结果），缺即 fail。

````
===== BEGIN STAGE-2-EXPLORER PROMPT =====
...
````
===== BEGIN STAGE-3-SYNTHESIZER PROMPT =====
...
2. 综合决策（**必选 1 个 4 态**）：
   - **CONTINUE-TO-CODING-RELAY**: 推荐继续 → 给 coding-relay 的 PLAN 草图（≤ 10 行：目标 / 主要方法 / 验收）
...
===== END STAGE-3-SYNTHESIZER PROMPT =====
````
...
- **`full-spike`**：3 段（默认）。开放问题 / 不熟领域 / 多个 probe 需结构化探索（≥ 2 probe 或依赖未验证或需运行实验）
- **`single-probe`**：Stage 1 退化为产 1 个 probe（不跳 Stage 1）→ Stage 2 仅跑该 probe → Stage 3 单 probe verdict + 4 态决策。适用：用户给的就是一个具体可观察问题 + 已有测量方式
- **`learning-only`**：跳 EXPLORER 写代码，仍跑 Stage 2 但**禁止写 spike code**——Probe Results 的 Raw Observations 引证文档原文 / 源码 file:line / 公开 benchmark URL，仍按 Measurement 跑可执行命令（grep / man / rg）记录原始输出。适用：问题不需要跑代码就能回答（如"X 库的 API 模型是什么 / X 协议规定 Y 怎么处理"）

降级 3 维度（可数）：**probe 数**（≥ 2 → full-spike；恰 1 → single-probe）/ **是否需要跑代码**（需运行 / 依赖未验证 → full-spike 或 single-probe；纯读源码 / 文档 / spec → learning-only）/ **判据复杂度**（多维度对比 → full-spike；单测量轴 → single-probe）。
