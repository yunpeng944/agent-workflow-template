...
- **preset**: 见 [docs/workflows.md](../docs/workflows.md) 「Role / Model 映射 / Vendor 字典」节；默认 `claude-codex`；`custom` (`--diagnoser-a=<m> --diagnoser-b=<m> --reconciler=<m>` 缺任一 fail-fast)
- **role slots**: DIAGNOSER-A (Stage 1) / DIAGNOSER-B (Stage 2，与 A 平行) / RECONCILER (Stage 3，从未参与 Stage 1/2)；三 stage **全部** fresh subagent；Stage 1/2 平行 dispatch 互不可见；Stage 3 第三个独立 fresh subagent
...
- 通用调度行为（优先级 / dispatch ledger / worktree 隔离 / single-role preset 例外）→ [docs/workflows.md](../docs/workflows.md)
