...
候选清单 6 个月内未被新 PR 触发或被工具豁免显式引用，下次 AGENTS.md 修补时删除。删除同样适用 `{{check命令}}` / `{{lint命令}}` 验证。
...
- 同一规则在 3 次以上 session 中被同一类 agent 违反（含 PR review 拦截、test 拦截、人工纠正）。
- 规则在 `{{lint命令}}` / `{{check命令}}` 中无对应 enforcement，且过去 30 天无任何 PR 引用过它。
