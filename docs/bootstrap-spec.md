...
- **Level 0**（未自治）：无主入口 / 无验证路径
- **Level 1**（初始骨架）：有主入口但局部规则 / 验证不全
- **Level 2**（部分自治）：高频任务 agent 可自恢复 + 执行
- **Level 3**（可持续自治）：上下文 + 边界 + 验证 + 失效识别 + 治理更新均稳定

> autonomy 是 emergent（Anthropic 2026），同一仓库在不同 oversight 下 Level 可不同。
