...
<!-- generated · do not edit · source: skills/wf-incident-rescue.md -->
...
**降级**：已知 root cause → `known-cause-fix`（跳 Stage 1/2 进 Stage 3+4）；git log < 10 commits 自上次 known-good → `no-bisect`；first-bad 改动小独立 → `revert-only`（仍需 reproducer 转绿验证）。
...
- 建议用户：收窄 anchor / 修 build flake / 改用 `no-bisect` simplification
...
**Handoff to Stage 3**：Status = FIRST-BAD-FOUND 才进 Stage 3；INCONCLUSIVE 走 Simplification `no-bisect` 或先解决 anchor / build 问题。
...
**Handoff to Stage 4**：拿到 Decision。若 REVERT 且影响面 = 无依赖 → 可走 `revert-only` simplification 跳 Stage 4 直接执行 revert + 跑 validate；其他情况进 Stage 4。
...
- **`full-rescue`**：4 阶段（默认）。失败现场 + 未知 root cause + 多 commit 历史
- **`known-cause-fix`**：跳过 Stage 1 + Stage 2，进 Stage 3 + 4。适用：root cause 已通过 `wf-second-opinion` 或人工分析定位。**Stage 3 输入替代格式**：把 Stage 3 prompt 中的 `STAGE-2 BISECT-RESULT` 块替换为以下 `KNOWN-CAUSE-DESCRIPTION` 块：
  ...
  ## Reproducer
  - 引用 STAGE-1 REPRODUCTION 块（若 Stage 1 也跳过，提供等价的 failing test 命令）
  ===== END KNOWN-CAUSE-DESCRIPTION =====
  ```
  Stage 3 输出格式不变（仍含 Decision、Fix Scope、Negative Test Design 等段）。Stage 4 验证步骤完全照跑（仍需 reproducer 转绿 + negative test + ./tasks.sh validate）
- **`no-bisect`**：跳 Stage 2，进 Stage 1 → Stage 3 → Stage 4。适用：git log 自 known-good 起 < 10 commits，肉眼读 commit 就能定位
- **`revert-only`**：跳过 Stage 3 forward-fix 设计 + Stage 4 实现部分，但**仍需走完 Stage 4 三项验证**。适用：first-bad commit 改动小、独立、无依赖。执行：直接 `git revert <SHA>` → 跑 Stage 1 reproducer 应**转绿**（若不转绿说明 revert 没真覆盖 root cause，必须升级回 `full-rescue` 走完整 Stage 3）→ 跑 `./tasks.sh validate`。**不允许跳过 reproducer 验证**——这是 revert 是否真有效的唯一证据。Negative test 设计在 revert-only 路径下默认豁免（reproducer 即起到 regression test 作用）

降级 3 维度：**root cause 已知性**（已知 → `known-cause-fix`；未知 → 必须 Stage 1）/ **git history 深度**（< 10 commits → `no-bisect`；≥ 10 → bisect）/ **first-bad 可回退性**（可独立 revert + 无后续依赖 → `revert-only`）。
